...
struct rtk_lsadc_pad_info {
	...
	uint				range_evt_cfg;
	uint				level_set[LSADC0_PAD_LEVEL_SET_NUMBER];
};
...
struct rtk_lsadc_state {
	...
	struct mutex			lock; /* proctect registers access */
	const struct rtk_lsadc_chip_info *chip_info;
...
static irqreturn_t lsadc0_interrupt_pad(int irq, void *data)
{
	...
	for (i = 0; i < 2; i++) {
		...
		for (j = 0; j < LSADC0_PAD_LEVEL_SET_NUMBER; j++) {
			...
			dev_dbg(st->dev, "[LSADC] pad%d level set%d reg = 0x%0x\n", i, j, reg);
			if ((reg & LSADC0_LEVEL_BLK_EN_MASK) &&
				...
				dev_dbg(st->dev, "[LSADC] pad%d level set%d INT!\n", i, j);
				iio_push_event(idev,
	...
	if (pad0_int) {
		...
		if (st->lsadc[0].pad[0].adc_val0 > new_adc_val) {
			...
			dev_dbg(st->dev, "[LSADC] pad0 adc became bigger!\n\n");
	...
	if (pad1_int) {
		...
		if (st->lsadc[0].pad[1].adc_val0 > new_adc_val) {
			...
			dev_dbg(st->dev, "[LSADC] pad1 adc became bigger!\n\n");
...
static irqreturn_t rtk_lsadc_trigger_handler(int irq, void *data)
{
	...
	iio_push_to_buffers_with_timestamp(idev, st->buffer,
					   iio_get_time_ns(idev));
			    ...
			    char *buf)
{
	...
	if ((ctrl_reg & LSADC0_CTRL_ENABLE_MASK) == 0) {
		...
		dev_dbg(dev, "[LSADC] write ctrl_enable, ctrl_reg=0x%x\n",
			ctrl_reg);
	}
	...
	for (i = 0; i < LSADC0_PAD_LEVEL_SET_NUMBER; i++) {
		...
		len += scnprintf(buf + len, PAGE_SIZE - len,
				 "set_idx[%d]: pad0_set=0x%0x, pad1_set=0x%0x\n",
			      ...
			      int *val, int *val2, long mask)
{
	...
	case IIO_CHAN_INFO_SCALE:
		*val = st->chip_info->ref_volt_mv;
		*val2 = st->chip_info->adc_max;
		ret = IIO_VAL_FRACTIONAL;
		break;
	case IIO_CHAN_INFO_ENABLE:
		if (chan->modified) {
			int lv = rtk_mod_map[chan->channel2];

			if (chan->channel > 1 || lv >= LSADC0_PAD_LEVEL_SET_NUMBER)
				goto err_exit;
			       ...
			       int val, int val2, long mask)
{
	...
	case IIO_CHAN_INFO_ENABLE:
		...
		if (chan->modified) {
			...
			if ((reg & LSADC0_LEVEL_BLK_EN_MASK) !=
			    (activate << LSADC0_LEVEL_BLK_EN_SHIFT)) {
				if (activate)
					reg |= LSADC0_LEVEL_BLK_EN_MASK;
				else
					reg &= ~LSADC0_LEVEL_BLK_EN_MASK;
				reg &= ~LSADC0_LEVEL_INT_PEND_MASK;
				st->lsadc[0].pad[chan->channel].level_set[lv] = reg;
				 ...
				 int state)
{
	...
	case IIO_EV_DIR_RISING:
		if (chan->modified) {
			...
			if (state) {
				reg |= LSADC0_LEVEL_INT_EN_MASK;
				st->lsadc[0].pad[chan->channel].range_evt_cfg |= BIT(lv);
			} else {
				reg &= ~LSADC0_LEVEL_INT_EN_MASK;
			}
			reg &= ~LSADC0_LEVEL_INT_PEND_MASK;
				...
				int val, int val2)
{
	...
	if (val < 0 || val > max_thresh) {
		dev_dbg(st->dev, "channel %d: val (%d) out of range (0~%u)\n",
	...
	if (chan->modified) {
		...
		switch (dir) {
		case IIO_EV_DIR_RISING:
			old_thresh = (reg & LSADC0_LEVEL_TOP_BOUND_MASK) >>
				LSADC0_LEVEL_TOP_BOUND_SHIFT;
			reg = (reg & ~LSADC0_LEVEL_TOP_BOUND_MASK) |
				(val << LSADC0_LEVEL_TOP_BOUND_SHIFT);
			break;
		case IIO_EV_DIR_FALLING:
			old_thresh = (reg & LSADC0_LEVEL_LOW_BOUND_MASK) >>
				LSADC0_LEVEL_LOW_BOUND_SHIFT;
			reg = (reg & ~LSADC0_LEVEL_LOW_BOUND_MASK) |
				(val << LSADC0_LEVEL_LOW_BOUND_SHIFT);
			break;
		default:
		...
		LSADC_WRITEL(reg, chan->address);
		mutex_unlock(&st->lock);
		dev_dbg(st->dev, "channel %d, mapped channel2 %d: write pad_reg = 0x%x, new threshold = %d, old threshold = %d\n",
			chan->channel, lv, reg, val, old_thresh);
	} else {
		if (chan->channel > 1)
			return -EINVAL;
		...
		LSADC_WRITEL(reg, chan->address);
		mutex_unlock(&st->lock);
		dev_dbg(st->dev, "channel %d: write pad_reg = 0x%x, new threshold = %d, old threshold = %d\n",
				...
				unsigned int *readval)
{
	...
	if (readval) {
		*readval = LSADC_READL(addr);
		dev_dbg(dev, "read reg 0x%X = 0x%X\n", addr, *readval);
	} else {
		LSADC_WRITEL(writeval, addr);
		dev_dbg(dev, "write reg 0x%X = 0x%X\n", addr, writeval);
	}
	mutex_unlock(&st->lock);
...
static void rtk_lsadc_hw_init(struct iio_dev *idev)
{
	struct rtk_lsadc_state *st = iio_priv(idev);
	uint ctrl_reg;
	uint analog_ctrl_reg;
	...
	reset_control_deassert(st->rstc);
	...
	dev_dbg(st->dev, "[LSADC] write lsadc0_power_reg=0x%x\n", lsadc_power_reg);
	...
	dev_dbg(st->dev, "[LSADC] pad0=[activate=%d, ctrl_mode=%d, pad_sw=%d, threshold=%d]\n",
		st->lsadc[0].pad[0].activate,
		st->lsadc[0].pad[0].ctrl_mode,
		st->lsadc[0].pad[0].pad_sw,
		st->lsadc[0].pad[0].threshold);
	dev_dbg(st->dev, "[LSADC] pad1=[activate=%d, ctrl_mode=%d, pad_sw=%d, threshold=%d]\n",
		st->lsadc[0].pad[1].activate,
		st->lsadc[0].pad[1].ctrl_mode,
		st->lsadc[0].pad[1].pad_sw,
		st->lsadc[0].pad[1].threshold);

	dev_dbg(st->dev, "[LSADC] ctrl_reg=0x%x, lsadc_status_reg=0x%x, pad0_reg=0x%x, pad1_reg=0x%x\n",
		ctrl_reg, lsadc_status_reg, pad0_reg, pad1_reg);

	for (i = 0; i < 2; i++) {
		dev_dbg(st->dev, "[LSADC] pad %d: range_evt_cfg=0x%x\n",
			i, st->lsadc[0].pad[i].range_evt_cfg);
		for (j = 0; j < LSADC0_PAD_LEVEL_SET_NUMBER; j++)
			dev_dbg(st->dev, "[LSADC] pad %d, level set %d: reg=0x%x\n",
				i, j, st->lsadc[0].pad[i].level_set[j]);
	}
	...
	LSADC_WRITEL(pad0_reg, LSADC0_PAD0_ADDR);
	dev_dbg(st->dev, "[LSADC] write pad0_reg=0x%x\n", pad0_reg);
	...
	LSADC_WRITEL(pad1_reg, LSADC0_PAD1_ADDR);
	dev_dbg(st->dev, "[LSADC] write pad1_reg=0x%x\n", pad1_reg);
	...
	LSADC_WRITEL(ctrl_reg, LSADC0_CTRL_ADDR);
	dev_dbg(st->dev, "[LSADC] write ctrl_reg=0x%x\n", ctrl_reg);
	...
	if ((lsadc_status_reg & LSADC0_STATUS_IRQ_EN_MASK) != LSADC0_STATUS_IRQ_EN_MASK) {
		...
		dev_dbg(st->dev, "[LSADC] write lsadc_status_reg=0x%x\n",
			lsadc_status_reg);
	}
	...
	dev_dbg(st->dev, "[LSADC] set ctrl_reg=0x%x, lsadc_status_reg=0x%x, pad0_reg=0x%x, pad1_reg=0x%x\n",
		ctrl_reg, lsadc_status_reg, pad0_reg, pad1_reg);

	/* Init CRT LSADC PG */
	writel(st->crt_lsadc_pg_val, st->crt_lsadc_pg_addr);
	dev_dbg(st->dev, "[LSADC] write crt_lsadc_pg_val=0x%x\n",
		st->crt_lsadc_pg_val);

	for (i = 0; i < 2; i++) {
		addr = (i ? LSADC0_PAD1_LEVEL_SET0_ADDR : LSADC0_PAD0_LEVEL_SET0_ADDR);
		for (j = 0; j < LSADC0_PAD_LEVEL_SET_NUMBER; j++) {
			reg = st->lsadc[0].pad[i].level_set[j];
			LSADC_WRITEL(reg, addr + 4 * j);
			dev_dbg(st->dev, "[LSADC] write pad%d level set%d reg = 0x%0x\n",
				i, j, reg);
		}
	}
}

static void rtk_lsadc_hw_deinit(struct iio_dev *idev)
{
	struct rtk_lsadc_state *st = iio_priv(idev);
	uint analog_ctrl_reg;
	uint ctrl_reg;
	...
	/* Power down JD_power */
	analog_ctrl_reg = LSADC_READL(LSADC0_ANALOG_CTRL_ADDR);
	analog_ctrl_reg &= ~LSADC0_ANALOG_CTRL_JD_PWR_MASK;
	LSADC_WRITEL(analog_ctrl_reg, LSADC0_ANALOG_CTRL_ADDR);

	/* Disable module enable */
	ctrl_reg = LSADC_READL(LSADC0_CTRL_ADDR);
	ctrl_reg &= ~LSADC0_CTRL_ENABLE_MASK;
	LSADC_WRITEL(ctrl_reg, LSADC0_CTRL_ADDR);

	clk_disable_unprepare(st->clk);
	reset_control_assert(st->rstc);
}
...
static int rtk_lsadc_suspend(struct device *dev)
{
	...
	dev_dbg(dev, "[LSADC] Enter %s\n", __func__);

	rtk_lsadc_hw_deinit(idev);

	dev_dbg(dev, "[LSADC] Exit %s\n", __func__);
...
static SIMPLE_DEV_PM_OPS(rtk_lsadc_pm_ops, rtk_lsadc_suspend, rtk_lsadc_resume);

#define RTK_LSADC_PM_OPS	(&rtk_lsadc_pm_ops)

#else /* CONFIG_PM_SLEEP */
#define RTK_LSADC_PM_OPS	NULL
#endif /* CONFIG_PM_SLEEP */

static const unsigned long rtk_lsadc_avail_scan_masks[] = {
	0x3, 0
};
...
static int rtk_lsadc_probe(struct platform_device *pdev)
{
	...
	st->rstc = devm_reset_control_get_exclusive(dev, NULL);
	if (IS_ERR(st->rstc)) {
		st->rstc = NULL;
	...
	idev->available_scan_masks = rtk_lsadc_avail_scan_masks;

	ret = devm_iio_triggered_buffer_setup(dev, idev, iio_pollfunc_store_time,
					      rtk_lsadc_trigger_handler, NULL);
	if (ret) {
		dev_err(dev, "failed to setup of iio triggered buffer: %d\n", ret);
		return ret;
	}

	st->trig = devm_iio_trigger_alloc(dev, "%s-dev%d", idev->name, 0);
	if (!st->trig) {
		dev_err(dev, "failed to allocate trigger\n");
		return -ENOMEM;
	}

	st->trig->ops = &rtk_lsadc_trigger_ops,
	...
	idev->trig = iio_trigger_get(st->trig);

	ret = devm_request_irq(dev, st->lsadc[0].irq, lsadc0_interrupt_pad, IRQF_SHARED,
			       dev_name(dev), idev);
	if (ret) {
	...
	dev_info(dev, "[LSADC] %s : IRQ %d\n", __func__, st->lsadc[0].irq);
...
static const struct rtk_lsadc_chip_info default_chip_info = {
	.adc_max = 64,
	.ref_volt_mv = 3300,
};

static const struct rtk_lsadc_chip_info rtd1619b_chip_info = {
	.adc_max = 255,
	.ref_volt_mv = 1800,
};

static const struct of_device_id rtk_lsadc_of_match[] = {
	{ .compatible = "realtek,rtk-lsadc0", .data = &default_chip_info, },
	{ .compatible = "realtek,rtd1619b-lsadc0", .data = &rtd1619b_chip_info, },
	{ }
};
