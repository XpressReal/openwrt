...
static void dwc3_rtk_debug_driver_add(struct dwc3_rtk *rtk)
{
	struct platform_device *debug;
	int ret = 0;

	debug = platform_device_alloc("rtk-dwc3-debug", PLATFORM_DEVID_AUTO);
	if (!debug) {
		dev_err(rtk->dev, "couldn't allocate debug device\n");
		return;
	}

	debug->dev.parent = rtk->dev;
	rtk->debug = debug;

	ret = platform_device_add(debug);
	if (ret) {
		dev_err(rtk->dev, "failed to register debug device\n");
		goto err;
	}

	return;
err:
	platform_device_put(debug);
}

static void dwc3_rtk_debug_driver_remove(struct dwc3_rtk *rtk)
{
	platform_device_unregister(rtk->debug);
}
...
static int dwc3_rtk_remove(struct platform_device *pdev)
{
	...
	dwc3_rtk_debug_driver_remove(rtk);
