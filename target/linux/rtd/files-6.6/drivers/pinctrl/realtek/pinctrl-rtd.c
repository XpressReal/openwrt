...
struct rtd_pin_vol_detect_desc {
	const char *name;
	unsigned int reg_offset;
...
#define RTK_PIN_VOL(_name, _reg_off, _en_off, _st_off, ...) \
	{ \
		.name = # _name, \
		.reg_offset = _reg_off, \
		.en_offset = _en_off, \
...
struct rtd_pinctrl_desc {
	...
	const struct rtd_pin_vol_detect_desc *vol_pins;
	unsigned int num_vol_pins;
};
...
#include "pinctrl-rtd1319d.h"
#include "pinctrl-rtd1315e.h"
...
static const struct of_device_id rtd_pinctrl_dt_ids[] = {
	...
	{ .compatible = "realtek,rtd13xxd-pinctrl", .data = &rtd1319d_iso_pinctrl_desc },
	{ .compatible = "realtek,rtd13xxe-pinctrl", .data = &rtd1315e_iso_pinctrl_desc },
