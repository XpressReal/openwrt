	    ...
	    struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (buf[0] == 'Q' || buf[0] == 'q') {
		...
		dev_dbg(dev, "%s curret eMMC protect region status:\n", __func__);
		for (args = 0; args < max_sectors; args += (wpg_unit * 0x20)) {
			dw_mci_rtk_query_protect_cmd(mmc, args, &src);
			pr_err("from sector 0x%lx = 0x%x\n", args, src);
		}
	} else if (buf[0] =='C' || buf[0] == 'c') {
		dev_dbg(dev, "%s clear all eMMC protect group regions...\n", __func__);
		if (!priv->protect_cnt) {
			for (args = 0; args < max_sectors; args += wpg_unit)
				dw_mci_rtk_write_protect_cmd(mmc, args, 0);
		} else {
			for (args = 0; args < priv->protect_cnt; args += wpg_unit)
				dw_mci_rtk_write_protect_cmd(mmc, priv->protect_start + args, 0);
		}
		priv->protect_cnt = 0;
	} else if (buf[0] == 'S' || buf[0] == 's') {
		dev_dbg(dev, "%s set the eMMC protect group regions...\n", __func__);
		if (!priv->protect_cnt) {
			dw_mci_rtk_write_protect_cmd(mmc, priv->protect_start, 1);
		} else {
			for (args = 0; args < priv->protect_cnt; args += wpg_unit)
				dw_mci_rtk_write_protect_cmd(mmc, priv->protect_start + args, 1);
		}
		priv->protect_cnt = 0;
	}
...
static ssize_t protect_region_cnt_dev_show(struct device *dev,
                                                struct device_attribute *attr, char *buf)
{
        struct dw_mci *host = dev_get_drvdata(dev);
        struct dw_mci_rtkemmc_host *priv = host->priv;
...
static ssize_t protect_region_cnt_dev_store(struct device *dev,
                struct device_attribute *attr, const char *buf, size_t count)
{
        struct dw_mci *host = dev_get_drvdata(dev);
        struct dw_mci_rtkemmc_host *priv = host->priv;
        ...
        ret = kstrtoul(buf, 0, &val);
        priv->protect_cnt = val;

        pr_err("protect_cnt = 0x%x\n", priv->protect_cnt);

        return count;
}

DEVICE_ATTR(protect_region_cnt, S_IRUGO | S_IWUSR,
                protect_region_cnt_dev_show, protect_region_cnt_dev_store);
...
static void setup_clk_div(struct dw_mci_slot *slot)
{
	...
	if (clock != host->current_speed) {
		...
		if (clock != slot->__clk_old) {
			...
			pr_info("Bus speed (slot %d) = %dHz (slot req %dHz, actual %dHZ div = %d)\n",
...
static int search_best(u32 window, u32 range)
{
	...
	if (window == HS200_WINDOW_ALL_PASS) {
		window_best = 0x10;
	}
...
static int dw_mci_rtk_execute_tuning(struct dw_mci_slot *slot, u32 opcode)
{
	...
	if(mmc->doing_retune == 1) {
		mmc->can_retune = 0;
		dev_info(mmc_dev(mmc), "%s: retune case.\n", __func__);
		goto out;
	}
	...
	do {
		...
		} while(rx_window == HS200_WINDOW_ALL_PASS && loop_cnt < 60);
	...
	do {
		...
		pr_info("check write protect region 0 during tuning: 0x%x\n", src);
		if(src & BIT(0))
		...
		else if((tx_window == HS400_WINDOW_ALL_PASS && mmc->ios.timing == MMC_TIMING_MMC_HS400) ||
		...
		(tx_window == HS200_WINDOW_ALL_PASS && mmc->ios.timing == MMC_TIMING_MMC_HS200)) && loop_cnt < 60);
...
static int dw_mci_rtk_parse_dt(struct dw_mci *host)
{
	...
	if (priv->emmc_mode == 3) {
		...
		prop = of_get_property(host->dev->of_node, "enforce", &size);
		if (prop) {
			priv-> enforce = of_read_number(prop, 1);
			printk(KERN_ERR "[%s] enforce value: %d\n", __func__, priv->enforce);
		}

		cell = nvmem_cell_get(host->dev, "emmc_pkg_cal");
		if (IS_ERR(cell)) {
			pr_info("cannot get the uuid info !\n");
			priv->emmc_mode = 3;
			goto out;
		}

		buf = nvmem_cell_read(cell, &buf_size);
		pr_info("%s: otp_emmc_pkg_cal = 0x%x\n", __func__, buf[0]);
		if ((buf[0] & BIT(0)) == 0 &&  priv->enforce == 0)
			priv->emmc_mode = 2;
		else
			priv->emmc_mode = 3;
...
static int dw_mci_rtk_probe(struct platform_device *pdev)
{
	...
	ret = device_create_file(&pdev->dev, &dev_attr_protect_region_cnt);
        if (ret < 0) {
                dev_err(&pdev->dev, "device_create_count_file fail (ret=%d)\n",
                        ret);
        }
	ret = device_create_file(&pdev->dev, &dev_attr_tuning_info);
        if (ret < 0) {
		dev_err(&pdev->dev, "device_create_tuning_info fail (ret=%d)\n",
			ret);
	}
