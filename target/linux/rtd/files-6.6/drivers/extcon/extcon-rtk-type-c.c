...
static int extcon_rtk_type_c_remove(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
...
static int extcon_rtk_type_c_prepare(struct device *dev)
{
	...
	/* disable cc detect, rp, rd */
	cc1_reg = type_c->reg_base + USB_TYPEC_CTRL_CC1_0;
	cc2_reg = type_c->reg_base + USB_TYPEC_CTRL_CC2_0;
	val = readl(cc1_reg);

	writel(PLR_EN | val, cc1_reg);
	writel(PLR_EN, cc1_reg);
	writel(0, cc2_reg);
