...
int aicbsp_set_subsys(int subsys, int state)
{
	...
	printk("== aicbsp_set_subsys: 169: state: %d ==\n", state);
	if (state) {
		cur_power_map |= (1 << subsys);
		printk("if(state): cur_power_map: %d \n", cur_power_map);
	} else {
		cur_power_map &= ~(1 << subsys);
		printk("if(state): cur_power_map: %d \n", cur_power_map);
	}
	...
	printk("== aicbsp_set_subsys: 177: state: %d ==\n", state);

	if (cur_power_state != pre_power_state) {
		printk("== aicbsp_set_subsys: 180: cur_power_state: %d, pre_power_state: %d ==\n", cur_power_state, pre_power_state);
		...
		if (cur_power_state) {
			if (aicbsp_platform_power_on() < 0) {
				printk("FUNC: aicbsp_set_subsys: condition: aicbsp_platform_power_on() < 0, goto err0  \n");
				goto err0;
			}
			printk("== set aicbsp_platform_power_on success \r\n\n");

			int ret = aicbsp_sdio_init();
			if (ret) {
				printk("FUNC: aicbsp_set_subsys: condition: aicbsp_sdio_init: ret: %d, goto err1 \n", ret);
				goto err1;
			}
			printk("== set aicbsp_sdio_init success ==\r\n\n");

			printk("== ready to set aicbsp_sdiodev ==\r\n\n");

			if(aicbsp_sdiodev  == NULL) {
			    printk("== aicbsp_sdiodev  is NULL ==\n");
				return -1;
			}

			if (aicbsp_driver_fw_init(aicbsp_sdiodev)) {
				printk("FUNC: aicbsp_set_subsys: condition: aicbsp_driver_fw_init(aicbsp_sdiodev), goto err2 \n");
				goto err2;
			}
			printk("== set aicbsp_driver_fw_init success ==\r\n\n");
...
err2:
	printk("== aicbsp_set_subsys: 234: this is err2 ==\n");
	aicbsp_sdio_release(aicbsp_sdiodev);
	aicbsp_sdio_exit();

err1:
	printk("== aicbsp_set_subsys: 239: this is err1 ==\n");
	aicbsp_platform_power_off();

err0:
	printk("== aicbsp_set_subsys: 243: this is err0 ==\n");
	...
	const struct sdio_device_id *id)
{
	...
	printk("== func is not null, continue ==\n");
	...
	printk("== before up aicbsp_probe_semaphore ==\n");
...
static int aicbsp_platform_power_on(void)
{
	...
	printk("\n == aicbsp_set_subsys: down_timeout start ==\n");
...
{
	printk("== aicbsp_sdio_init start ==\n");
	if (sdio_register_driver(&aicbsp_sdio_driver)) {
		printk("\n== %s: sdio_register_driver failed ==\r\n", __func__);
		return -1;
	} else {
