...
// 	const u8 *buf,
// 	size_t len)
 static int rwnx_cfg80211_tdls_mgmt(struct wiphy *wiphy,
                                    struct net_device *dev,
                                    const u8 *peer,
                                    int link_id,
                                    u8 action_code,
...
{
	printk("== FUNC: rwnx_mod_init, start ==\n");
	RWNX_DBG(RWNX_FN_ENTRY_STR);
	rwnx_print_version();
    rwnx_init_cmd_array();

//#ifndef CONFIG_PLATFORM_ROCKCHIP
	printk("== ready to set AIC_PWR_ON ==\n");
	if (aicbsp_set_subsys(AIC_WIFI, AIC_PWR_ON) < 0) {
		AICWFDBG(LOGERROR, "== %s, set power on fail! ==\n", __func__);
		if(!aicbsp_get_load_fw_in_fdrv()){
			printk("== FUNC: aicbsp_get_load_fw_in_fdrv: failed to load fw fdrv ==\n");
			return -ENODEV;
		}
	}
	printk("== set AIC_PWR_ON end ==\n");
//#endif
	...
	if ((wait_for_completion_timeout(&hostif_register_done, msecs_to_jiffies(REGISTRATION_TIMEOUT)) == 0) || rwnx_driver_err) {
		...
		printk("== ready to set AIC_PWR_OFF ==\n");
        aicbsp_set_subsys(AIC_WIFI, AIC_PWR_OFF);
		printk("== set AIC_PWR_OFF end ==\n");
		return -ENODEV;
	}
