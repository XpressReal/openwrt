...
irqreturn_t rtd13xx_handle_wrapper_msi_irq(int this_irq, void *devid)
{
	...
	val = readl(pp->ctrl_base + PCIE_MSI_DATA);
	if (val & 0x10000)
...
static int rtd_pcie_wrapper_msi_init(struct rtd_pcie_port *pp)
{
	...
	struct soc_device_attribute rtk_13xx_soc[] = {
		{
			.family = "Realtek Hank",
		},
		{
	...
	soc_att_match = soc_device_match(rtk_13xx_soc);
	...
	if (pp->msi_irq) {
		if (soc_att_match) {
			ret = request_irq(pp->msi_irq, rtd13xx_handle_wrapper_msi_irq, IRQF_SHARED, pp->ops->name, pp);
			if (ret) {
...
static int pcie_link_init(struct rtd_pcie_port *pp)
{
	...
	if (pp->speed_mode == 0) { /* force gen1*/
		...
		dev_info(pp->dev, "Host Set Link Speed on Gen1\n");
	}
	...
	case 0x5:
		...
		break;
	default:
		dev_info(pp->dev, "Current Link Speed: not define(0x%x)", cur_link_speed);
		break;
	}

	phy_calibrate(pp->pcie_phy);
...
static int rtd_pcie_probe(struct platform_device *pdev)
{
	...
	if (pp->ops->get_ca_type) {
		pp->ops->get_ca_type(pp);
	}
	...
	list_for_each_entry(child_dev, &bus->devices, bus_list) {
		u32 mps;
		mps = 128 << child_dev->pcie_mpss;
		if (mps >= 256) {
			ret = pcie_set_mps(child_dev, 256);
			if (ret)
				break;
			else
				writel((readl(pp->ctrl_base + 0x78) & ~0xe0)| 0x20, pp->ctrl_base + 0x78);
		}
	}
	...
	list_for_each_entry(child_dev, &bus->devices, bus_list) {
		aspm_support = 0;
		pcie_capability_read_dword(child_dev, PCI_EXP_LNKCAP, &child_lnkcap);
		pcie_capability_read_word(child_dev, PCI_EXP_LNKCTL, &child_lnkctl);
		...
		aspm_support &= pp->aspm_support;

		pcie_capability_clear_and_set_word(child_dev, PCI_EXP_LNKCTL,
					   PCI_EXP_LNKCTL_ASPMC, aspm_support);
	}
	tmp = readl(pp->ctrl_base + LINK_CONTROL_LINK_STATUS_REG);
	writel((tmp & ~0x11) | aspm_support, pp->ctrl_base + LINK_CONTROL_LINK_STATUS_REG);
#endif
...
static int rtd13xx_pcie0_get_res(struct rtd_pcie_port *pp)
{
	...
	prop = of_get_property(pp->dev->of_node, "aspm", &size);
	if (prop) {
		pp->aspm_support = of_read_number(prop, 1);
	} else {
		pp->aspm_support = 0;
	}
...
static int get_service_region_setting(struct rtd_pcie_port *pp, u32 *start, u32 *end)
{
	u32 size = 0, start_addr, end_addr;
	int ret;

	ret  = of_property_read_u32_index(pp->dev->of_node, "service-region", 0, &start_addr);
	if (ret) {
		dev_err(pp->dev, "get service region start addr failed");
		return ret;
	}
	ret  = of_property_read_u32_index(pp->dev->of_node, "service-region", 1, &size);
	if (ret) {
		dev_err(pp->dev, "get service region size failed");
		return ret;
	}

	end_addr = start_addr + size;

	if (start_addr & 0x7fffff || end_addr & 0x7fffff) {
		dev_err(pp->dev, "service region not align\n");
		return -EINVAL;
	}
...
static int set_pcie_service_region(struct rtd_pcie_port *pp)
{
	bool is_cas = false;
	...
	ret = get_service_region_setting(pp, &start, &end);
	if (ret) {
		start = 0x0;
		end = 0x0;
	}
	if (is_cas) {
		arm_smccc_smc(RTK_SET_PCIE_SERVICE_REGION, start, end, 0, 0, 0, 0, 0, &res);
	} else {
		const u32 *prop;
		int size = 0;
		prop = of_get_property(pp->dev->of_node, "service-region-enable", &size);
		if (prop) {
			val = BIT(24) | ((end >> 23) << 15)  | ((start >> 23) << 3);
		} else {
			val = 0;
		}
		writel(val, pp->ctrl_base + PCIE_SERVICE_REGION);
	}
	return 0;
}
...
static int rtd13xxd_pcie1_init(struct rtd_pcie_port *pp)
{
	u32 tmp;
	int ret = 0;
...
static void rtd_get_ca_type(struct rtd_pcie_port *pp)
{
	...
	if (of_find_property(np, "nvmem-cell-names", NULL)) {
		...
		cell = nvmem_cell_get(pp->dev, "ca-type");
		if (!IS_ERR(cell)) {
			unsigned char *buf;
			size_t buf_size;
			...
			dev_err(pp->dev, "missing nvmem resource\n");
		}
	} else {
		dev_dbg(pp->dev, "can't find nvmem cell node\n");
		pp->ca_type = 0;
	}
}
...
static const struct rtd_pcie_ops rtd13xxd_pcie1_ops = {
	...
	.msi_ops = &rtd_mac_msi_ops,
	.get_resource = rtd13xx_pcie1_get_res,
	.init = rtd13xxd_pcie1_init,
	.deinit = rtd13xx_pcie1_deinit,
	.hwinit = rtd16xxb_pcie1_hw_init,
	.get_ca_type = rtd_get_ca_type,
};
