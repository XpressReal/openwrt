...
static int __init buflock_module_init (void)
{
	...
	module->dev_class = class_create(MODULE_NAME);
	...
	do {
		dma_addr_t dma_addr;
		void *vaddr;
		size_t size = sizeof(unsigned int) * module->max_handles;

		vaddr = dma_alloc_coherent(module->device, size, &dma_addr,
					 GFP_KERNEL);

		if (!vaddr)
			goto out;

		module->uStatusPhyAddr = (unsigned int) dma_addr;
		module->pStatusMemory = (buflock_status_t *)vaddr;
        } while (0);
...
static void __exit buflock_module_exit(void)
{
	...
	size = sizeof(unsigned int) * module->max_handles;
	dma_addr = module->uStatusPhyAddr;
	vaddr = module->pStatusMemory;
	dma_free_coherent(module->device, size, vaddr, dma_addr);
    }
...
static ssize_t buflock_fops_read (struct file *file, char *buf, size_t size, loff_t *f_pos)
{
    ...
    do  {
        ...
        s += snprintf(s, size, "#%s phy=%08zx max=%zu checkloop_ms=%d"
			"\n destroy_timeout_ms=%d\n", MODULE_NAME,
			 (long unsigned int) module->uStatusPhyAddr,
			 module->max_handles,
			 module->release_loop_ms, module->destroy_timeout_ms);

        s += snprintf(s, size, "    clients:\n");
        mutex_lock(&module->clients_lock);
        {
            struct buflock_client * client, * tmp_client;
            list_for_each_entry_safe(client,  tmp_client, &module->clients, list) {
                s += snprintf(s, size, "        %u %u [%s]\n", client->pid, client->tgid, client->task_comm);
                mutex_lock(&client->mutex);
                {
                    struct buflock_slot * slot, * tmp_slot;
                    list_for_each_entry_safe(slot,  tmp_slot, &client->slots, list) {
        ...
        s += snprintf(s, size, "    inactive handle:\n");
        {
            struct buflock_handle * handle, * tmp_handle;
            list_for_each_entry_safe(handle,  tmp_handle, &module->free_handles, list) {
                unsigned int lastSet_delta_us;
                ...
                s += snprintf(s, size, "        %p (id=%08x, %.8s) lastSet(%5d.%03d ms), Destroy(%5d.%03d ms)\n", handle,
                        handle->getFWID(handle), getStatusStr(handle->getStatus(handle)),
                        lastSet_delta_us/1000, lastSet_delta_us%1000,
