...
static irqreturn_t hse_interrupt(int irq, void *dev_id)
{
	...
	for (i = 0; i < hse_dev->num_eng; i++)
		hse_engine_handle_interrupt(&hse_dev->eng[i]);
	return IRQ_HANDLED;
}
...
static int hse_probe(struct platform_device *pdev)
{
	...
	hse_dev->num_eng = hse_dev->quirks->num_eng;
	hse_dev->eng = devm_kcalloc(dev, hse_dev->num_eng, sizeof(*hse_dev->eng), GFP_KERNEL);
	if (!hse_dev->eng)
		return -ENOMEM;
	...
	dev_info(dev, "initialized wtih %d engine(s), dev=%s, dmaengine=%s\n", hse_dev->num_eng,
		hse_dev->miscdevice_ready ? "Y" : "N", hse_dev->dmaengine_ready ? "Y" : "N");
...
static const struct hse_engine_desc simple_eng_desc[] = {
	{ .offset = 0x300, }
};

static const struct hse_quirks rtd1319_quirks = {
	.bypass_en_disable = 1,
	.xor_copy_v2 = 0,
	.eng_desc = simple_eng_desc,
	.num_eng = ARRAY_SIZE(simple_eng_desc),
};
...
static const struct hse_engine_desc rtd1319d_eng_desc[] = {
	{ .offset = 0x300, },
	{ .offset = 0x700, .type = HSE_ENGINE_MODE_RCMD, }
};

static const struct hse_quirks rtd1319d_quirks = {
	.bypass_en_disable = 1,
	.xor_copy_v2 = 1,
