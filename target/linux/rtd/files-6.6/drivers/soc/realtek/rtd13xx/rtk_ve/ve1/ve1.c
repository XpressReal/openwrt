...
{
	struct reset_control *rstc = coreIdx ? rstc_ve1 : rstc_ve3;
...
static void vpu_hw_reset_init(struct device *dev)
{
	rstc_ve1 = devm_reset_control_get_exclusive(dev, "ve1");
	if (IS_ERR(rstc_ve1))
		dev_warn(dev, "failed to get ve1 reset control: %ld\n", PTR_ERR(rstc_ve1));
	rstc_ve3 = devm_reset_control_get_exclusive(dev, "ve3");
	if (IS_ERR(rstc_ve3))
		dev_warn(dev, "failed to get ve3 reset control: %ld\n", PTR_ERR(rstc_ve3));
}

int rtd13xx_vpu_alloc_dma_buffer(vpudrv_buffer_t *vb)
{
	if (!vb)
		return -1;
	...
	pr_err("%s base:0x%lx, phy_addr:0x%lx, size:%d\n", DEV_NAME, vb->base, vb->phys_addr, vb->size);
	return 0;
}

__maybe_unused
static int rtd13xx_vpu_alloc_dma_buffer2(vpudrv_buffer_t *vb)
{
	if (!vb)
		return -1;
...
void rtd13xx_vpu_free_dma_buffer(vpudrv_buffer_t *vb)
{
	if (!vb)
		return;
...
static int vpu_free_buffers(struct file *filp)
{
	...
	list_for_each_entry_safe(pool, n, &s_vbp_head, list) {
		if (pool->filp == filp) {
			vb = pool->vb;
			if (vb.base) {
				rtd13xx_vpu_free_dma_buffer(&vb);
				list_del(&pool->list);
				kfree(pool);
			}
		}
...
void rtd13xx_vpu_clock_getting(vpu_clock_info_t *clockInfo)
{
	if (clockInfo->enable) {
...
int rtd13xx_vpu_down_interruptible(void)
{
	return down_interruptible(&s_vpu_sem);
}

void rtd13xx_vpu_sem_up(void)
{
	up(&s_vpu_sem);
}

void rtd13xx_vpu_open_ref_count_inc(void)
{
	s_vpu_open_ref_count++;
}

void rtd13xx_vpu_open_ref_count_dec(void)
{
	s_vpu_open_ref_count--;
}

void rtd13xx_vpu_add_vbp_list(vpudrv_buffer_pool_t *vbp, struct file *filp)
{
	vbp->filp = filp;
	spin_lock(&s_vpu_lock);
...
int rtd13xx_vpu_open_inst(vpudrv_inst_info_t *inst_info, struct file *filp)
{
	vpudrv_instanace_list_t *vil, *n;
...
void rtd13xx_vpu_close_inst(vpudrv_inst_info_t *inst_info)
{
	vpudrv_instanace_list_t *vil, *n;
...
void rtd13xx_vpu_get_inst_num(vpudrv_inst_info_t *inst_info)
{
	vpudrv_instanace_list_t *vil, *n;
...
void rtd13xx_vpu_get_total_inst_num(vpudrv_inst_info_t *inst_info)
{
	vpudrv_instanace_list_t *vil, *n;
...
void rtd13xx_vpu_free_mem(vpudrv_buffer_t *vb)
{
...
int rtd13xx_vpu_wait_init(vpu_drv_context_t *dev, vpudrv_intr_info_t *info)
{
	int ret = 0;
	unsigned long flags;
	...
	if (info->core_idx == 0) {
		...
		ret = wait_event_interruptible_timeout(s_interrupt_wait_q_ve3,
						       atomic_read(&s_interrupt_flag_ve3) != 0,
						       msecs_to_jiffies(info->timeout));
		if (!ret)
			return -ETIME;
...
vpudrv_buffer_t *rtd13xx_vpu_get_instance_pool(void)
{
	return &s_instance_pool;
}

vpudrv_buffer_t *rtd13xx_vpu_get_common_memory(void)
{
	return &s_common_memory;
}

vpudrv_buffer_t *rtd13xx_vpu_get_vpu_register(void)
{
	return &s_vpu_register;
}

vpudrv_buffer_t *rtd13xx_vpu_get_bond_register(void)
{
	return &s_bond_register;
}

vpudrv_buffer_t *rtd13xx_vpu_get_dc_register(void)
{
	return &s_dc_register;
}

vpudrv_buffer_t *rtd13xx_vpu_get_dmc_register(void)
{
	return &s_dmc_register;
}

int rtd13xx_vpu_alloc_from_vm(void)
{
	int ret = 0;
...
int rtd13xx_vpu_alloc_from_dmabuffer2(void)
{
	int ret = 0;

	if (rtd13xx_vpu_alloc_dma_buffer2(&s_instance_pool) != 0) {
		up(&s_vpu_sem);
		return -ENOMEM;
	}
...
static long vpu_ioctl(struct file *filp, u_int cmd, u_long arg)
{
	...
	case VDI_IOCTL_ALLOCATE_PHYSICAL_MEMORY:
	{
		...
		ret = rtd13xx_vpu_down_interruptible();
		if (ret != 0)
			return -EFAULT;
		...
		ret = rtd13xx_vpu_alloc_dma_buffer(&(vbp->vb));
		if (ret != 0) {
			kfree(vbp);
			rtd13xx_vpu_sem_up();
			return -ENOMEM;
		...
		rtd13xx_vpu_add_vbp_list(vbp, filp);
	...
	case VDI_IOCTL_WAIT_INTERRUPT:
	{
		...
		ret = rtd13xx_vpu_wait_init(dev, &info);
		if (ret != 0)
			return -EFAULT;
	...
	case VDI_IOCTL_GET_COMMON_MEMORY:
	{
		...
		if (s_common_memory.base != 0) {
			...
			ret = rtd13xx_vpu_alloc_dma_buffer(&s_common_memory);
			if (ret != 0)
				return -ENOMEM;
	...
	case VDI_IOCTL_OPEN_INSTANCE:
	{
		...
		ret = rtd13xx_vpu_open_inst(&inst_info, filp);
		if (ret)
			return -ENOMEM;

		/* flag just for that vpu is in opened or closed */
		rtd13xx_vpu_open_ref_count_inc();
	...
	case VDI_IOCTL_CLOSE_INSTANCE:
	{
		...
		rtd13xx_vpu_close_inst(&inst_info);

		/* flag just for that vpu is in opened or closed */
		rtd13xx_vpu_open_ref_count_dec();
	...
	case VDI_IOCTL_GET_INSTANCE_NUM:
	{
		...
		rtd13xx_vpu_get_inst_num(&inst_info);
	...
	case VDI_IOCTL_SET_RTK_CLK_GATING:
	{
		...
		rtd13xx_vpu_clock_getting(&clockInfo);
	...
	case VDI_IOCTL_GET_TOTAL_INSTANCE_NUM:
	{
		...
		if (ret)
			return -EFAULT;
		rtd13xx_vpu_get_total_inst_num(&inst_info);
	...
	if (ret == 0) {
		...
		if (s_vpu_drv_context.open_count == 0) {
...
#else
				rtd13xx_vpu_free_dma_buffer(&s_instance_pool);
#endif /* USE_VMALLOC_FOR_INSTANCE_POOL_MEMORY */
			...
			if (s_common_memory.base) {
				...
				rtd13xx_vpu_free_dma_buffer(&s_common_memory);
