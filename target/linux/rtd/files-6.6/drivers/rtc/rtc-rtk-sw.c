...
static void sw_rtc_restore_from_ptime(struct sw_rtc_data *data)
{
	time64_t time;
	struct rtc_time tm;
	int ret;

	ret = rtk_ptime_load_time(data->dev, &time);
	if (ret)
		return;

	rtc_time64_to_tm(time, &tm);
	dev_info(data->dev, "restore time from ptime-reg: %04d.%02d.%02d %02d:%02d:%02d",
...
static void sw_rtc_save_to_ptime(struct sw_rtc_data *data)
{
	struct rtc_time tm;
	time64_t time;
	int ret;
	...
	ret = rtk_ptime_store_time(data->dev, time);
	if (ret)
		return;

	dev_info(data->dev, "save time to ptime-reg: %04d.%02d.%02d %02d:%02d:%02d",
		1900 + tm.tm_year, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour,
		tm.tm_min, tm.tm_sec);
}
...
static int sw_rtc_probe(struct platform_device *pdev)
{
	...
	sw_rtc_restore_from_ptime(data);
...
static void sw_rtc_shutdown(struct platform_device *pdev)
{
	struct sw_rtc_data *data = platform_get_drvdata(pdev);

	sw_rtc_save_to_ptime(data);
}
